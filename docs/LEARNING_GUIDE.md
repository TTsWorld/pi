# 学习指南 — pi monorepo（commit `f064ea0e1`）
...
1. **`src/index.ts`**（仅 5 行占位）
2. **`packages/ai/plan.md`**（950 行设计蓝图）— 统一 `AI` 类、`stream()` 流式优先、统一 Event 格式
3. 三份 API 调研笔记（openai-api.md / anthropic-api.md / gemini-api.md，共 6200+ 行）——可跳读，对照 plan.md 看各家差异

> 💡 学习要点：这是一个「文档先行」的样本——此刻 ai 包代码为零，但蓝图已完整。后续提交会看到它逐步吸收 agent.ts 的方言代码。
